...
export default function AdminAccessRequests() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Permintaan Akses Dokumen</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">Kelola permintaan akses dokumen terbatas</p>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-700/30">
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-500 dark:text-slate-500">Tidak ada permintaan akses.</td></tr>
              ) : requests.map((r) => (
                <tr key={r.id} className="hover:bg-white dark:bg-slate-700/20 transition-colors">
                  <td className="px-4 py-3"><div className="text-slate-800 dark:text-slate-200 font-medium">{r.user_name}</div><div className="text-xs text-slate-500 dark:text-slate-500">{r.user_email}</div></td>
                  <td className="px-4 py-3"><div className="text-slate-800 dark:text-slate-200 max-w-40 truncate" title={r.archive_title}>{r.archive_title}</div><div className="text-xs text-slate-500 dark:text-slate-500 capitalize">{r.archive_category?.replace(/_/g," ")}</div></td>
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-400 max-w-40 truncate" title={r.reason}>{r.reason}</td>
                  <td className="px-4 py-3"><Badge status={r.status} /></td>
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{new Date(r.requested_at).toLocaleDateString("id-ID")}</td>
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{r.expired_at ? new Date(r.expired_at).toLocaleDateString("id-ID") : "—"}</td>
        ...
        <div className="space-y-4">
          {actionModal.request && (
            <div className="p-3 bg-white dark:bg-slate-700/40 rounded-lg text-sm">
              <div className="text-slate-700 dark:text-slate-300 font-medium">{actionModal.request.user_name}</div>
              <div className="text-slate-600 dark:text-slate-400 text-xs mt-1">{actionModal.request.archive_title}</div>
              <div className="text-slate-600 dark:text-slate-400 text-xs mt-1 italic">"{actionModal.request.reason}"</div>
            </div>
          )}
